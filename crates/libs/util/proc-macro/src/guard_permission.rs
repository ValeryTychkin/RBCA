...
use syn::{
    parse_macro_input, parse_quote, parse_str, spanned::Spanned, Expr, FnArg, Ident, ItemFn,
    LitStr, Meta, Pat, PatIdent, PatType, Path, Type, TypePath,
};
...
fn snake_to_camel(snake: &str, upper_first: bool) -> String {
...
fn gen_handle_guard_struct(
    ...
    quote! {
        ...
        struct #handle_ident(#guard_ident);
...
pub fn impl_guard_permission(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let guard_ident =
        match add_handle_guard(&mut item_fn, &handle_ident, &macro_args.guard_arg_name) {
            Ok(v) => v,
            Err(e) => {
                return TokenStream::from(e.write_errors());
            }
